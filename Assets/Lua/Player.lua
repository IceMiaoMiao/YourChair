...
function this.Update()
	if Input.GetKeyDown(KeyCode.Space) or Input.GetMouseButtonDown(0) then
		particle : GetComponent('ParticleSystem') : Play()
		startTime = Time.time--获取按下空格时的时间
	end

	if Input.GetKeyUp(KeyCode.Space) or Input.GetMouseButtonUp(0) then
		--DoTween恢复人物
		Util.DoScale(body, 0.9, 0.5)
		Util.DoLocalMoveZ(head, 0.001, 0.5)
		particle : GetComponent('ParticleSystem') : Stop()
		endTime = Time.time - startTime--计算按下空格至松开的时间
		this.StartJump(endTime)
	end

	if Input.GetKey(KeyCode.Space) or Input.GetMouseButton(0) then
		--人物压缩效果
